...
(function () {
  ...
  var tower, monsters, camera, renderer;
  ...
  function run() {
    Game.Load.images(IMAGES, function (images) {
      Game.Load.json("levels/basic", function (level) {
        setup(images, level);
        Game.run({
          fps: FPS,
          update: update,
  ...
  function render(dt) {
    if (isPaused) {
      return; // Si el juego está en pausa, no renderices nada
    }
    renderer.render(dt);
  }
  ...
  var Player = Class.create({
    initialize: function (tower) {
      this.x = col2x(0.5);
      this.y = row2y(0);
      console.log("Posición inicial del jugador:", this.x, this.y);
      console.log("Fila inicial del jugador:", y2row(this.y)); // Depuración
      this.w = PLAYER_WIDTH;
    ...
    update: function (dt) {
      if (this.paused || isPaused) {
        // Si el juego está pausado, no actualices la posición ni el estado de las teclas
        this.dx = 0;
        this.dy = 0;
        return;
      }
      ...
      // Verificar si el jugador ha alcanzado la meta (fila más alta del mapa)
      const playerRow = y2row(this.y);

      // Ajustar la condición para que solo se active cuando el jugador esté en la fila más alta (tower.rows - 1)
      if (playerRow === tower.rows - 1 && this.y >= row2y(tower.rows - 1)) {
        console.log("¡El jugador ha alcanzado la meta!");
        this.reachGoal(); // Llamar a la función cuando el jugador alcanza la meta
        return; // Detener la actualización del jugador
      }
    ...
    hitMonster: function () {
      ...
      if (window.renderer) {
        window.renderer.renderScore(); // Llama a la función que actualiza el puntaje y las vidas
      }

      // Verificar si hay preguntas disponibles
      if (availableQuestions.length === 0) {
        // No hay preguntas disponibles, simplemente continuar el juego
        if (this.lives <= 0) {
          this.gameOver(); // Si no tiene vidas, terminar el juego
        }
        return true;
      }

      // Si hay preguntas disponibles, mostrar una pregunta matemática
      if (this.lives <= 0) {
        this.savedX = this.x; // Guarda la posición X
        this.savedY = this.y; // Guarda la posición Y
        this.showMathQuestion(); // Muestra una pregunta matemática
      }
      return true;
    },

    showMathQuestion: function () {
      this.paused = true; // Pausa el juego
      ...
      const questionImage = document.getElementById("math-question-image");
      if (!questionImage) {
        console.error("No se encontró el elemento 'math-question-image'.");
        return;
      }
      questionImage.src = currentQuestion.questionImage;

      // Configurar las imágenes de las opciones de respuesta
      const options = document.querySelectorAll(
        "#math-answer-options button img"
      );
      if (options.length === 0) {
        console.error("No se encontraron opciones de respuesta.");
        return;
      }

      currentQuestion.options.forEach((option, index) => {
        if (options[index]) {
          options[index].src = option.image;
        } else {
          console.error(
            `No se encontró la opción de respuesta en el índice ${index}.`
          );
        }
      });

      // Mostrar el modal de la pregunta
      const questionModal = document.getElementById("math-question-modal");
      if (!questionModal) {
        console.error("No se encontró el modal de preguntas.");
        return;
      }
    ...
    resetGame: function () {
      ...
      // Reinicia los monstruos
      monsters.all.forEach((monster) => {
        monster.x = col2x(monster.col + 0.5);
        monster.y = row2y(monster.row);
      });
      ...
      // Oculta el modal de preguntas matemáticas
      document.getElementById("math-question-modal").style.display = "none";

      // Reinicia el estado de pausa
      isPaused = false;

      // Limpiar la lista de preguntas respondidas
      localStorage.removeItem("answeredQuestions");
      answeredQuestions = []; // Reiniciar la lista en memoria
    },
    ...
    reachGoal: function () {
      ...
      showMessageModal(
        "¡Felicidades!",
        `¡Ganaste! Puntos obtenidos: ${this.score}`
      );
  ...
  var Monsters = Class.create({
    initialize: function (level) {
      ...
      this.savedX = this.x; // Inicializa la posición X guardada
      this.savedY = this.y; // Inicializa la posición Y guardada
    },
    ...
    update: function (dt) {
      if (isPaused) {
        return; // Si el juego está en pausa, no actualices al enemigo
      }
      var n,
        max,
        all = this.all;
  ...
  var Renderer = Class.create({
    ...
    renderScore: function (ctx) {
      ...
      this.vscore = player.score;
      Dom.set(score, this.vscore);

      // Actualizar las vidas
      Dom.set(lives, player.lives); // Actualiza el contador de vidas
    },
